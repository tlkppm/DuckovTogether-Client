        ...
        try
        {
            ...
            if (list != null)
            {
                ...
                for (var i = 0; i < ordered.Count; i++)
                {
                    ...
                    if (mod.IsServer)
                    {
                        var sceneId = AIRequest.Instance.GetRootSceneId(__instance);
                        bool useNewSystem = false;
                        
                        if (Main.AI.AIInstanceSync.Instance != null)
                        {
                            try
                            {
                                Main.AI.AIInstanceSync.Instance.Server_RegisterAIInstance(cmc.gameObject, sceneId);
                                useNewSystem = true;
                                Debug.Log($"[AI-SYNC] 使用新系统同步 AI: {cmc.name} 场景:{sceneId}");
                            }
                            catch (System.Exception ex)
                            {
                                Debug.LogWarning($"[AI-SYNC] 新系统失败，回退到旧系统: {ex.Message}");
                            }
                        }
                        ...
                        if (!useNewSystem)
                        {
                            ...
                            Debug.Log($"[AI-SEED] 使用旧系统(备用)同步 AI: aiId={aiId}");
                        }
                    }
                }
